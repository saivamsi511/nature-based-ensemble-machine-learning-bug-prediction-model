...
--
-- Database: `nature_based_prediction_model`
--
CREATE DATABASE IF NOT EXISTS `nature_based_prediction_model` DEFAULT CHARACTER SET latin1 COLLATE latin1_swedish_ci;
USE `nature_based_prediction_model`;
...
--
-- Table structure for table `remote_user_bug_report_prediction`
--

CREATE TABLE IF NOT EXISTS `remote_user_bug_report_prediction` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `bug_id` varchar(300) NOT NULL,
  `creation_date` varchar(300) NOT NULL,
  ...
  `resolution_category` varchar(300) NOT NULL,
  `status_category` varchar(300) NOT NULL,
  `bug_fix_time` varchar(300) NOT NULL,
  `Prediction` varchar(300) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB  DEFAULT CHARSET=latin1 AUTO_INCREMENT=7 ;

--
-- Dumping data for table `remote_user_bug_report_prediction`
--

INSERT INTO `remote_user_bug_report_prediction` (`id`, `bug_id`, `creation_date`, `component_name`, `product_name`, `short_description`, `long_description`, `reporter_name`, `resolution_category`, `status_category`, `bug_fix_time`, `Prediction`) VALUES
(1, 'XORG-2701', '11-03-05', 'Driver/intel', 'XORG', 'Switch to console or other session changes resolution\r\n', '"Using the current CVS sources for the i810 driver results in another resolution\r\nthan the initial resolution (when Xserver starts) after switching to the text\r\nconsole and back. I''ll attach a logfile and the Xserver output during after\r\nswitching to text console and back."\r\n', 'sndirsch', 'fixed', 'closed', '4', 'normal'),
(2, 'LIBREOFFICE-69169', '10-09-13', 'Presentation', 'LIBREOFFICE', 'SLIDESHOW: Crash when using some transition effects\r\n', '"Libreoffice crashes when I press F5.\r\n\r\nIt doesn''t crash for newly created files but it crashes on each ppt(x)s that I tried.\r\n\r\nIt hangs when I pressed F5 using a converted odp (from ppt).\r\n\r\nVersion: LibreOffice 4.1.1.2 410m0(Build:2)\r\nDistro: Arch Linux\r\nKernel: Linux 3.10.10-1-ARCH #1 SMP PREEMPT Fri Aug 30 11:30:06 CEST 2013 x86_64 GNU/Linux"\r\n', 'wmzhere', 'fixed', 'resolved', '60', 'major'),
(3, 'TELEPATHY-27967', '04-05-10', 'tp-glib', 'TELEPATHY', 'telepathy-glib master fails check/distcheck since enums branch was merged\r\n', '"Subject says it all see the referenced branch.\r\n\r\n(Danielle: if these changes have been merged by the time you start tomorrow please have a look at them in git so you know for future work)"\r\n', 'smcv', 'fixed', 'resolved', '0', 'blocker'),
(4, 'MESA-42069', '20-10-11', 'Drivers/Gallium/r300', 'MESA', 'Texture artifacts\r\n', '"Created attachment 52603\r\nscreenshot\r\n\r\nr300g git (Mesa 7.12-devel (git-6235846))\r\nMobility X1600 / 128mb\r\n\r\nr300g has some texture artifacts not there on r600g nor llvmpipe.\r\nSince the texture coords are calculated in the vertex shader it''s possible the bug is in the interpolation?\r\n\r\nApitrace:\r\nhttp://kiwi6.com/file/25hl2953ab"\r\n', 'cand', 'fixed', 'resolved', '449', 'minor'),
(5, 'TELEPATHY-17072', '11-08-08', 'haze', 'TELEPATHY', 'Remove snpp from haze.manager\r\n', 'haze.manager is made on my system which has the snpp prpl but in general that prpl doesn''t exist so it shouldn''t be listed.\r\n', 'will', 'fixed', 'resolved', '7', 'normal'),
(6, 'LIBREOFFICE-82128', '04-08-22', 'Spreadsheet', 'LIBREOFFICE', 'FILESAVE: Select Protected/Unprotected Cells status is lost\r\n', '"Problem description: The 2 options Select protected/unprotected cells are inverted.\r\nOperating System: Windows 7\r\nVersion: 4.3.0.4 release\r\nLast worked in: 4.2.5.2 release"\r\n', 'jojo', 'fixed', 'resolved', '11', 'normal');
...
(18, 'SVM', '74.36708860759494'),
(19, 'Logistic Regression', '76.89873417721519'),
(20, 'Decision Tree Classifier', '73.89240506329115'),
(21, 'KNeighborsClassifier', '78.48101265822784'),
(22, 'Random Forest Classifier', '78.95569620253164');
...
(47, 'normal', '50.0'),
(48, 'blocker', '16.666666666666664'),
(49, 'major', '16.666666666666664'),
(50, 'minor', '16.666666666666664');
